...
def warm_up_per_epoch(Guesser, Ranker, d_optimizer,
    ...
    for batch_idx, inputs in tqdm(enumerate(data_loader),
        ...
        real_data, masked_data, refer_data = inputs
        ...
        generate_data = sample(model=Guesser, stoi=data_loader.dataset.stoi, max_seq_len=max_seq_len,
                               mask=masked_data, batch_size=batch_size, device=device)
...
def val_per_epoch(Guesser, data_loader, device, epoch_num):
    ...
    for batch_idx, inputs in tqdm(enumerate(data_loader), total=len(data_loader), desc=f'validating epoch {epoch_num}'):
        passwords, masked_passwords = inputs[0]
        prob = calculate_probability(model=Guesser, password=passwords, masked_password=masked_passwords, stoi=data_loader.dataset.stoi, device=device)
        try:
    ...
    top_prob_dict = {}
    top_prob = 0.0
    for top in data_loader.dataset.top_10:
        masked_passwords = top["masked_password"]
        pwd = top["guessed_password"]
        log_prob = 0.0
        for i in range(len(pwd)):
            log_prob += -math.log(
                calculate_probability(model=Guesser, password=pwd[i], masked_password=masked_passwords,
                                      stoi=data_loader.dataset.stoi, device=device), 10.0)
        log_prob /= len(pwd)
        top_prob_dict[masked_passwords] = log_prob
        top_prob += log_prob
...
if __name__ == '__main__':
    ...
    val_dataloader = DataLoader(val_dataset, batch_size=1, shuffle=False, num_workers=0, collate_fn=lambda batch: batch)
